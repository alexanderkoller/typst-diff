...
use crate::context_recording::RecordedAlignment;
use crate::decision::{DecisionEvent, DecisionSink, FallbackCode, bounded_preview};
use crate::diff_area::DiffAreaKind;
use crate::diff_surface::{DiffSelection, DiffSurfaceKind};
use crate::edit_script::{EditScriptMode, EditScriptOp, build_edit_script};
...
fn replace_block_edit(
    ...
    let selection = select_modified_fragment(
    ...
    let selected_area = selection.as_ref().map(|selection| selection.area());
    let selected_surface_kind = selection.as_ref().map(|selection| selection.surface_kind());
    let edits = selection
        .map(|selection| vec![RealizedEdit::WholeBlock(selection.into_content())])
        .unwrap_or_default();
    ...
    emit_pipeline_trace_event(
        ...
        PipelineTraceEvent::new("diff/replace-block", "selected")
            .reason(format!(
                ...
                selected_area
                    .map(DiffAreaKind::trace_name)
                    .unwrap_or(DiffAreaKind::BodyBlock.trace_name()),
...
fn modified_fragment_edit_content(
    ...
    .map(DiffSelection::into_content)
}

fn select_modified_fragment(
    area: DiffAreaKind,
    old_content: &Content,
    new_content: &Content,
    old_ann: Option<&AnnotatedContent>,
    ...
    old_equation_origins: &[Content],
    new_equation_origins: &[Content],
) -> Option<DiffSelection<EditContent>> {
    if let Some(content) = raw_block_modified_content(old_content, new_content, old_ann, new_ann) {
        return Some(DiffSelection::new(area, DiffSurfaceKind::RawLines, content));
    }
...
fn balanced_word_annotations<'a>(
    ...
    let old_key = old_ann.and_then(|node| content_key::block_context_key(&node.realized));
    let new_key = new_ann.and_then(|node| content_key::block_context_key(&node.realized));
...
fn block_context_changed(
    ...
    match (
        content_key::block_context_key_for(old, old_ann),
        content_key::block_context_key_for(new, new_ann),
    ) {
...
fn is_empty_block_shell(content: &Content) -> bool {
    content.plain_text().trim().is_empty() && content_key::is_block_context(content)
}
            ...
            IndexedBlockOp::Equal(old_index, new_index) => {
                ...
                if presentation_changed(
                    ...
                    if (block_context_changed(
                        ...
                        old_ann,
                        new_ann,
                    ) || content_key::semantic_heading_context(old_ann, new_ann)
                        || (content_key::is_block_context(&old_block.content)
                            && content_key::is_block_context(&new_block.content)))
                        && replacement_has_word_change(
                            &old_block,
                            &new_block,
                            old_ann,
                    ...
                    let selected_surface_kind =
                        selection.as_ref().map(|selection| selection.surface_kind());
                    let edits = selection
                        .map(|selection| vec![RealizedEdit::WholeBlock(selection.into_content())])
                        .unwrap_or_default();
                    if !edits.is_empty() {
                        emit_pipeline_trace_event(
                            ...
                            PipelineTraceEvent::new("diff/equal-block", "selected")
                                .reason(format!(
                                    "area={}; presentation identity changed; surface={}",
...
fn diff_region_edits(old: &AnnotatedContent, new: &AnnotatedContent) -> Vec<RealizedEdit> {
    ...
    modified_fragment_edit_content(
        DiffAreaKind::SemanticPageRegion,
        &old_effective,
        &new_effective,
        Some(old),
        Some(new),
    ...
    let pages = new_pages
        ...
        .map(|(index, new_page)| {
            ...
            let selection = DiffSelection::new(
                DiffAreaKind::RenderedPageRegion,
            ...
            let word_ops = selection.into_content();
    ...
    old_page
        ...
        .map(|(old_cluster, new_cluster)| {
            ...
            RenderedRegionSegmentEdit {
                ...
                word_ops: selection.into_content(),
            }
        })
        .collect()
}
...
fn rendered_region_structural_wrapper(
    ...
    rendered_region_alignment(world, content).map(RenderedRegionWrapper::Align)
}

fn rendered_region_alignment(
    world: &dyn World,
    content: &Content,
    ...
    if let Some(context) = content.to_packed::<ContextElem>() {
        return crate::context_recording::syntax_alignment(world, context.span())
            .map(rendered_region_recorded_alignment);
    }
    if let Some(styled) = content.to_packed::<StyledElem>() {
        return rendered_region_alignment(world, &styled.child);
    }
    if let Some(seq) = content.to_packed::<SequenceElem>() {
        let mut found = None;
        for child in &seq.children {
            if let Some(alignment) = rendered_region_alignment(world, child)
...
fn rendered_region_recorded_alignment(alignment: RecordedAlignment) -> RenderedRegionAlignment {
    match alignment {
        RecordedAlignment::Left => RenderedRegionAlignment::Left,
        RecordedAlignment::Center => RenderedRegionAlignment::Center,
        RecordedAlignment::Right => RenderedRegionAlignment::Right,
        RecordedAlignment::Start => RenderedRegionAlignment::Start,
        RecordedAlignment::End => RenderedRegionAlignment::End,
    }
}
...
fn child_sequence_structural_edit_content(
    ...
    let new_keys: Vec<content_key::ContentKey> = new_children
        ...
        .map(|(_, child)| content_key::structural_child_key(child))
        .collect();
    let ops = build_edit_script(&old_keys, &new_keys, EditScriptMode::Lcs);
...
fn owned_surface_modified_edit(
    ...
    modified_fragment_edit_content(
        DiffAreaKind::StructuredContainerRegion,
        &old_effective,
        &new_effective,
        Some(old_ann),
        Some(new_ann),
...
fn diff_slot_edits_same_shape(
    ...
    diff_slot_edits_by_script(old_ann, new_ann, EditScriptMode::SameShape)
}

fn diff_slot_edits_by_script(
    old_ann: &AnnotatedContent,
    new_ann: &AnnotatedContent,
    mode: EditScriptMode,
) -> Vec<RealizedEdit> {
    ...
    let old_keys = slot_edit_keys(&old_slots);
    let new_keys = slot_edit_keys(&new_slots);
    let ops = build_edit_script(&old_keys, &new_keys, mode);
    let mut edits = Vec::new();

    for op in ops {
        push_slot_script_edits(&mut edits, &old_slots, &new_slots, op);
    }

    edits
}

fn slot_edit_keys(slots: &[(&SemanticSlot, &AnnotatedContent)]) -> Vec<content_key::ContentKey> {
    slots
        .iter()
        .map(|(_, child)| content_key::slot_child_match_key(child))
        .collect()
}

fn push_slot_script_edits(
    edits: &mut Vec<RealizedEdit>,
    ...
    match op {
        EditScriptOp::Equal {
            ...
            for i in 0..len {
                push_paired_slot_edit(
                    edits,
                    old_slots[old_index + i].1,
                    new_slots[new_index + i].1,
        ...
        EditScriptOp::Delete {
            ...
            for i in 0..old_len {
                push_deleted_slot_edit(edits, old_slots[old_index + i].1, new_slots, new_index);
            }
        }
        EditScriptOp::Insert {
            new_index, new_len, ..
...
fn push_paired_slot_edit(
    edits: &mut Vec<RealizedEdit>,
    old_child: &AnnotatedContent,
    new_child: &AnnotatedContent,
    new_slot: &SemanticSlot,
    skip_equal: bool,
) {
    if skip_equal && annotated_subtree_equal(old_child, new_child) {
        return;
    }
    if let Some(content) = recursive_slot_edit_content(old_child, new_child) {
        ...
        push_modified_slot_edit(edits, old_child, new_child, new_slot);
    }
}
...
fn modified_edit_content(
    ...
    if let (Some(old_caption), Some(new_caption)) = (
        old_child.realized.to_packed::<FigureCaption>(),
        new_child.realized.to_packed::<FigureCaption>(),
    ) {
        let old_tokens = extract_words(&old_caption.body);
        let new_tokens = extract_words(&new_caption.body);
        if old_tokens != new_tokens {
            return Some(EditContent::Modified {
                base: new_child.realized.clone(),
...
fn diff_slot_edits_lcs(
    ...
    diff_slot_edits_by_script(old_ann, new_ann, EditScriptMode::Lcs)
}
...
mod tests {
    ...
    fn diff_slot_edits_same_shape_recurses_to_arbitrary_depth() {
        ...
        let RealizedEdit::ReplaceAt {
            ...
            panic!("expected inner wrapper edit");
        };
    ...
    #[test]
    fn diff_slot_edits_same_shape_recurses_into_two_changed_nested_containers() {
        use crate::annotated::{
            AnnotatedContent, Annotation, SemanticKind, SemanticSlot, SlotStep, WrapperKind,
        };

        let make_leaf = |text: &str| AnnotatedContent {
            realized: TextElem::packed(text),
            annotation: Annotation::default(),
            children: vec![],
        };
        let make_list = |text: &str| AnnotatedContent {
            realized: TextElem::packed(text),
            annotation: Annotation {
                semantic_kind: Some(SemanticKind::List),
                slots: vec![SemanticSlot {
            ...
            children: vec![make_leaf(text)],
        };
        let make_wrapper = |first: &str, second: &str| AnnotatedContent {
            realized: TextElem::packed("wrapper"),
            annotation: Annotation {
                semantic_kind: Some(SemanticKind::Wrapper(WrapperKind::Block)),
                slots: vec![SemanticSlot {
            ...
            children: vec![AnnotatedContent {
                realized: TextElem::packed("body"),
                annotation: Annotation::default(),
                children: vec![make_list(first), make_list(second)],
            }],
        };

        let result = diff_slot_edits_same_shape(
            &make_wrapper("Old alpha", "Old beta"),
            &make_wrapper("New alpha", "New beta"),
        );
        let RealizedEdit::ReplaceAt {
            path,
            content: EditContent::Nested { edits, .. },
        ...
        assert_eq!(
            edits.len(),
            2,
            "both nested list changes should be preserved"
        );
        for (index, edit) in edits.iter().enumerate() {
            let RealizedEdit::ReplaceAt {
                ...
                panic!("expected nested list edit");
            };
            assert_eq!(path, &vec![index]);
            assert!(matches!(
                list_edits[0],
                RealizedEdit::ReplaceAt {
                    path: ref p,
