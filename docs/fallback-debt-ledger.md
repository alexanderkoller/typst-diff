...
## FB-005 Slot-Bearing Descendant Pair

- Warning code: `FB-005-slot-bearing-descendant-pair`
- Status: `active`
- Current source sites: `src/diff.rs` slot-bearing descendant matching below wrapper/body nodes.
- Why this is a guess: a unique descendant match is inferred after Typst inserts wrapper scaffolding.
- User-visible risk: nested wrappers with repeated bodies can be edited at the wrong descendant.
- Runtime warning behavior: not yet instrumented.
- Debug/debug-trace event names: planned `fallback/slot-bearing-descendant-pair`.
- Tests: wrapper/table/list regression tests in `tests/integration.rs`.
- Replacement abstraction: `content_tree` path mapping from authored wrapper body to realized descendant path.
- Removal criteria: descendant mapping is derived from explicit owner/path provenance or a typed unsupported result.
